...
4. `roles/<role-id>/main.md` — at minimum, an Owner role; add others if the project clearly warrants them
5. `roles/<role-id>/required-readings.yaml` — one file per role created in step 4. Read `$INSTRUCTION_REQUIRED_READINGS` before drafting. Populate each file with only the startup documents that role actually needs.
6. `roles/<role-id>/ownership.yaml` — one file per role created in step 4. Read `$INSTRUCTION_OWNERSHIP` before drafting. Populate each file with the surfaces that role is accountable for keeping current.
...
Before drafting documents 1–13, read the corresponding instruction by resolving `$INSTRUCTION_*` variables via `$A_SOCIETY_PUBLIC_INDEX`. Documents in steps 12–13 are initialized from `$GENERAL_IMPROVEMENT*` templates.
