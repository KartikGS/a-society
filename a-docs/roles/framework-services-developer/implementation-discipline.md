...
**Service-contract changes require co-maintenance visibility.** When a framework-service change alters a stable contract consumed by `general/`, `a-docs/`, or operator-facing executable docs, flag the corresponding truth-owning and registration surfaces in the completion report. Do not treat code-only completion as sufficient when the contract has changed.
