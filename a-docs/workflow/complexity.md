...
- **Parallel same-base-role tracks require numbered role instances:** If two nodes with the same base role may be active at the same time, use distinct role names such as `Curator_1` and `Curator_2` in `workflow.yaml`. Node notes like "Curator A" or "Curator B" are descriptive only; they do not create separate runtime sessions.
