...
**Status:** Framework active — Curator maintenance bundle complete; remaining next priorities all require Owner briefing before entry.
...
3. `[S]` — **Retirement scope in briefings** — When a briefing includes "retire a folder," scope should automatically include verifying and cleaning stale entries in `$A_SOCIETY_AGENT_DOCS_GUIDE`. Target: `$A_SOCIETY_WORKFLOW` Phase 4 or `$INSTRUCTION_AGENT_DOCS_GUIDE`; may fold into Priority 2 flow. Source: `$A_SOCIETY_RECORDS/20260308-records-infrastructure/05-owner-findings.md` (Finding 2). *Requires Owner briefing; may fold into Priority 2.*
