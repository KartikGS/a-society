...
**Status:** Framework active at v17.6. `backward-pass-quality-principles` (Tier 2) added Analysis Quality subsection to `$GENERAL_IMPROVEMENT` and `$A_SOCIETY_IMPROVEMENT` backward pass protocols — two principles: externally-caught errors require deeper root-cause analysis ("the rule was documented" is the start, not the end); a genuine finding must name a specific root cause, not describe what went wrong. Framework update report published at v17.6 (1 Recommended). Backward pass pending for `workflow-mechanics-corrections`, `next-priorities-bundle`, `graph-validator-human-collaborative`, `general-lib-sync-bundle`, `log-restructure`, and `backward-pass-quality-principles`.
