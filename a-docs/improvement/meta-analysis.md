...
- If you own a standing local surface in the distributed `ownership.yaml` registry, update it during this phase when the needed change is clear.
...
- Changes to `general/`, upstream runtime/tooling behavior, or other A-Society-deliverable surfaces that are not locally owned fixes belong in the final feedback artifact, not as direct edits here.
...
The Owner will decide what belongs in the final framework-feedback artifact. Your job here is to flag the candidate clearly, not to route it yourself.
...
6. **agents.md scope check:** Does `agents.md` contain anything beyond: what the project is (one paragraph), the authority/conflict resolution model, and project-wide invariants? If yes, flag it for removal.
