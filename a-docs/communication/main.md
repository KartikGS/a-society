...
Artifacts created during a flow belong in the runtime-provided active record folder under `.a-society/state/<project>/<flow-id>/record/`. The communication folder holds the stable references that make those artifacts consistent across flows.
...
`$A_SOCIETY_WORKFLOW` defines phase order and routing. `$A_SOCIETY_RUNTIME_RECORDS_CONTRACT` defines flow artifact placement and lifecycle. This communication folder defines the formats and protocols that make those handoffs readable and governable.
