**Subject:** Update report draft submission — records ordering guardrails
...
Implementation in this flow modified `$INSTRUCTION_RECORDS` and `$GENERAL_IMPROVEMENT` in ways adopting projects may need to review. This qualifies under `$A_SOCIETY_UPDATES_PROTOCOL` because an existing `general/` instruction changed in a way that affects the guidance adopting projects received, and the records instruction gained new mandatory guidance.

---

## What and Why

An update report is warranted for this change batch. The records instruction now carries three new adopter-facing rules: a context-dependent parallel-track sub-labeling rule, a mandatory workflow-path completeness rule, and explicit `workflow.md` exemptions for pre-convention and bootstrapping cases. The general improvement document also now requires an explicit forward-pass closure boundary before backward-pass work begins.

These changes affect how adopting projects define their records conventions and how they initiate backward-pass reflection. Projects that use records and `workflow.md` should review them directly. Projects without records or backward-pass tooling may only need selective adoption, but the changes still belong in the framework update stream so Curators can make that judgment explicitly.

---

## Where Observed

A-Society — internal. The need surfaced during the `backward-pass-orderer-redesign` follow-up work and was implemented in this flow as approved maintenance to the general library.

---

## Target Location

- Proposed publication target: `$A_SOCIETY_UPDATES_DIR/2026-03-21-records-ordering-guardrails.md`
...
**Files changed:** `$A_SOCIETY_RECORDS`, `$INSTRUCTION_RECORDS`, `$GENERAL_IMPROVEMENT`
**Publication condition outstanding:** Yes — Owner approval of this draft is still required. If approved, publish the report to `$A_SOCIETY_UPDATES_DIR/2026-03-21-records-ordering-guardrails.md` and update `$A_SOCIETY_VERSION` from `v16.0` to `v17.0` as a single atomic registration step.
...
The Curator does not publish the update report or update `$A_SOCIETY_VERSION` until the Owner approves this submission.
