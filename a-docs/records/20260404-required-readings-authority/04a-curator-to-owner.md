# Curator → Owner: Proposal (Required Readings Authority)
...
1.  **General role templates beyond owner, curator, and TA:** Confirmed only `owner.md`, `curator.md`, and `technical-architect.md` exist in `a-society/general/roles/`. All three are scoped for update.
2.  **$INSTRUCTION_AGENTS update:** Scoped to remove the `universal_required_reading` frontmatter and update the "Required reading list" and "Context confirmation" sections to mark them as legacy/human-orientation only.
3.  **$INSTRUCTION_ROLES update:** Scoped to remove `required_reading` frontmatter and update the `## Context Loading` section guidance.
4.  **Initializer assessment:** The Initializer (`$A_SOCIETY_INITIALIZER_ROLE`) must be updated to populate `required-readings.yaml` during bootstrap. This is scoped as a direct-authority item.
5.  **Role key naming convention:** The instruction will specify that role keys must be lowercase, hyphenated strings matching the role name as registered in the `agents.md` roles table (minus the "Agent" suffix if present, or exactly matching the lookup ID used by the runtime).
...
- *Rationale:* Since the runtime handles injection and role announcement, these sections are redundant and prone to drift.

### 3. Instruction Updates
**Target:** `$INSTRUCTION_AGENTS`
- Remove `## YAML Frontmatter: Universal Required Reading` section.
- Update `Required reading list` and `Context confirmation statement` to state they are for manual/human orientation and provide legacy/theater warnings.

**Target:** `$INSTRUCTION_ROLES`
- Remove `## YAML Frontmatter: Role-Specific Required Reading` section.
- Update `## Context Loading` section to reflect its role as human-orientation only.
- Update all Archetype templates to remove frontmatter and `Context Loading` prose.
...
- **A-Society Instance:** Create `a-society/a-docs/roles/required-readings.yaml` containing A-Society's universal and role-specific readings.
...
## Review Choice (Implementation Plan)

I will proceed to implement these changes upon approval. No Proposal artifact is required per the topology waiver in the workflow plan (Phase 0 design gate cleared).

Next action: Owner review.
Read: `a-society/a-docs/records/20260404-required-readings-authority/04a-curator-to-owner.md`
Expected response: `05-owner-review-decision.md` in the same record folder.
