...
**Implementation complete:** Yes. All Phase 3 and Phase 4 work complete. `$GENERAL_IMPROVEMENT` changes implemented and registered.

**Files changed (general/ only — update report scope):**
- `$GENERAL_IMPROVEMENT` (`a-society/general/improvement/main.md`) — two changes: synthesis routing step 5 replacement (Item 2c); backward pass tool mandate addition (Item 3b)

**Publication condition outstanding:** Yes — Owner approval required before publishing to `$A_SOCIETY_UPDATES_DIR` and incrementing `$A_SOCIETY_VERSION` to v17.3.

---

## Draft Update Report

*(Content below to be published as `a-society/updates/2026-03-21-workflow-mechanics-corrections.md` upon Owner approval. `$A_SOCIETY_VERSION` to be incremented to v17.3 at the same time.)*
...
- **APPROVED** — Curator publishes the draft above to `$A_SOCIETY_UPDATES_DIR` and increments `$A_SOCIETY_VERSION` to v17.3
- **REVISE** — with specific changes required before publication
- **REJECTED** — with rationale; no publication
