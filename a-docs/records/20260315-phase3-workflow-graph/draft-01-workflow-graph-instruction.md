...
**Step 3 — Designate the synthesis role.** Identify the role that produces synthesis in the backward pass (always fires last). Set `is_synthesis_role: true` on that role's final firing node. Set `false` on all others.
