---
**Subject:** Update report draft — index path rule and backward pass handoff completeness
**Status:** PROPOSED
**Date:** 2026-03-21
...
**Implementation status:** Complete. All three target files modified; no new files created.

**Files changed:**
| File | Change |
|---|---|
| `$INSTRUCTION_INDEX` | Path format rule replaced; unregistered-file guidance added; all example paths updated to repo-relative format |
| `$GENERAL_IMPROVEMENT` | Handoff completeness guardrail added to Guardrails section |
| `$A_SOCIETY_IMPROVEMENT` | Same guardrail added (MAINT, cross-layer consistency — Curator authority) |

**Publication condition:** One condition outstanding. `$A_SOCIETY_VERSION` increment (v17.0 → v17.1) is held pending Owner approval of this report. Per protocol, the version increment and report publication are a single atomic registration step. No other conditions blocking publication.

---

## Update Report Draft

> Proposed publication path: `a-society/updates/2026-03-21-index-paths-bp-handoffs.md`
