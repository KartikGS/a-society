...
Implementation of `03-owner-to-curator.md` (APPROVED) is complete. Changes to `general/` qualify under the update report trigger conditions: existing instructions and templates changed in ways that affect guidance adopting projects received at initialization; mandatory status introduced for a previously-deferrable artifact.

---

## What and Why

Six changes to `$INSTRUCTION_IMPROVEMENT`, `$INSTRUCTION_WORKFLOW`, and `$GENERAL_IMPROVEMENT` affect adopting projects. Three are Breaking: the synthesis path was corrected (old model said findings re-enter the main workflow as trigger inputs; correct model is a two-path lightweight loop), the two-file improvement structure was collapsed into a single `main.md` (retiring `$GENERAL_IMPROVEMENT_PROTOCOL`), and the improvement folder is now a required initialization artifact (previously deferrable). Three are Recommended: a precise backward pass traversal algorithm replaces the vague "walk from terminal to entry" description, the workflow instruction's backward pass section was updated from "(recommended)" to "(mandatory)", and guidance for flagging generalizable findings was added.

Breaking changes create active gaps or contradictions in initialized projects' existing improvement docs. Recommended changes improve precision and alignment with the new model.

---

## Where Observed

A-Society — internal. Changes originate from Owner brief and six design decisions in `01-owner-to-curator-brief.md`.

---

## Target Location

`/a-society/updates/2026-03-14-improvement-folder-redesign.md` — publish after Owner approval.
...
**Files changed:**
- `$INSTRUCTION_IMPROVEMENT` — full rewrite
- `$INSTRUCTION_WORKFLOW` — three targeted edits (backward pass ordering paragraph, Section 6, Step 7)
- `$GENERAL_IMPROVEMENT` — merged with retired protocol template, full rewrite
- `$GENERAL_IMPROVEMENT_PROTOCOL` — retired (file deleted)
- `$A_SOCIETY_IMPROVEMENT` — merged with retired protocol file, full rewrite (MAINT)
- `$A_SOCIETY_IMPROVEMENT_PROTOCOL` — retired (file deleted) (MAINT)
- `$A_SOCIETY_INDEX` — removed two protocol variable rows; updated `$GENERAL_IMPROVEMENT` and `$A_SOCIETY_IMPROVEMENT` descriptions (MAINT)
- `$A_SOCIETY_AGENT_DOCS_GUIDE` — improvement folder section updated to single-file rationale (MAINT)

**Publication condition outstanding:** Yes — `$A_SOCIETY_VERSION` has not yet been incremented to v8.0. Pending Owner approval of this report. Version increment and publication are a single atomic step.

---

## Owner Confirmation Required

The Owner must respond in `05-owner-update-report.md` with one of:
- **APPROVED** — Curator publishes the report to `$A_SOCIETY_UPDATES_DIR` and increments `$A_SOCIETY_VERSION` to v8.0
