...
Implementation of the nine changes to `$INSTRUCTION_WORKFLOW` approved in `03-owner-to-curator.md` is complete. The changes qualify for a framework update report under the following trigger conditions:
- An existing `general/` instruction has changed in a way that affects the guidance adopting projects received at initialization
- A new mandatory section has been added to a general instruction (session model: recommended → mandatory for two or more roles)

---

## What and Why

Nine improvements to `$INSTRUCTION_WORKFLOW` were made in this flow, ranging from a Breaking reclassification (session model mandatory) to Recommended structural additions (Owner as terminal node, backward pass section, session reuse rules) to Optional new patterns (parallel fork/join). Adopting projects whose workflow documents were initialized from the prior instruction may have gaps in session model coverage and will benefit from the other additions.

---

## Cross-Layer Drift Note (A-Society-internal — not in update report)

The standing check against `$A_SOCIETY_WORKFLOW` found three drift items that are out of scope for this flow but should be addressed in a future maintenance flow:

1. **At-flow-close session rule missing.** `$A_SOCIETY_WORKFLOW`'s "When to start a new session" section covers only the within-flow exhaustion/staleness criteria. The new at-flow-close rule (start fresh sessions for new flows) is absent.
2. **Copyable output language.** The transition description at human's role (lines 181–185) covers the substance but does not use the copyable artifact path / copyable session-start prompt framing now standard in `$INSTRUCTION_WORKFLOW` and all role documents.
3. **Owner-as-terminal not explicitly stated.** The A-Society workflow correctly ends at the Owner in practice, but the explicit principle is not stated.

These are in-scope for a future `[MAINT]` flow on `$A_SOCIETY_WORKFLOW`. They do not block this flow or the update report.

---

## Where Observed

A-Society — internal. Changes were made to `$INSTRUCTION_WORKFLOW` in `a-society/general/`.

---

## Target Location

`$A_SOCIETY_UPDATES_DIR` — `/a-society/updates/2026-03-14-workflow-instruction-improvements.md` (upon approval)
...
*End of draft update report.*

---

## Implementation Status

**Implementation complete:** Yes — `$INSTRUCTION_WORKFLOW` has been updated with all nine changes. See `03-owner-to-curator.md` for the approved change list.
**Files changed:** `$INSTRUCTION_WORKFLOW`
**Publication condition outstanding:** Yes — Owner approval of this update report is required before publishing to `$A_SOCIETY_UPDATES_DIR` and incrementing `$A_SOCIETY_VERSION` to v6.0.
...
- **APPROVED** — Curator publishes the report to `$A_SOCIETY_UPDATES_DIR` and updates `$A_SOCIETY_VERSION`
- **REVISE** — with specific changes required before resubmission
- **REJECTED** — with rationale

The Curator does not publish or increment version until `05-owner-to-curator.md` shows APPROVED status.
