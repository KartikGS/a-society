...
If an instruction only applies when a specific workflow type is active or a specific phase has been reached, it belongs in the workflow or phase-specific document - not in the role document. The workflow or phase document is not just the storage location; it is the delivery mechanism for that instruction. A role document that contains TA advisory review instructions will be read by agents in flows that have no TA, and a role document that merely points to TA review guidance without the workflow surfacing it still relies on memory rather than true phase-entry delivery.
