...
export function validateGraph(doc, strict) {
    const errors = [];
    ...
    if (!Array.isArray(wf.nodes) || wf.nodes.length === 0) {
        ...
        wf.nodes.forEach((node, i) => {
            ...
            if ('human-collaborative' in node) {
                if (typeof node['human-collaborative'] !== 'string' ||
                    node['human-collaborative'].trim() === '') {
                    errors.push(`workflow.nodes[${i}].human-collaborative must be a non-empty string if present`);
                }
            }
        });
        // workflow.edges
    ...
    if (strict) {
        ...
        if (workflow.edges.length === 0 && workflow.nodes.length === 1) {
            ...
            const startNodes = workflow.nodes.filter((node) => !toIds.has(node.id));
            const endNodes = workflow.nodes.filter((node) => !fromIds.has(node.id));
            for (const node of startNodes) {
                if (node.role !== 'Owner') {
                    errors.push(`Strict mode violation: start node "${node.id}" must have role "Owner" (found "${node.role}")`);
                }
            }
            for (const node of endNodes) {
                if (node.role !== 'Owner') {
                    errors.push(`Strict mode violation: end node "${node.id}" must have role "Owner" (found "${node.role}")`);
                }
            }
        }
    }
 ...
 */
export function validateWorkflowFile(filePath, strict) {
    let content;
    try {
