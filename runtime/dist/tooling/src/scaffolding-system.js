 ...
 */
export function detectFeedbackConsentType(entryPath) {
 ...
 */
export function titleFromPath(entryPath) {
 ...
 */
export function renderStub(entry) {
 ...
 */
export function scaffold(projectRoot, projectName, aSocietyRoot, entries, options = {}) {
    if (!projectRoot)
        throw new Error('projectRoot is required');
    if (!projectName)
        throw new Error('projectName is required');
    if (!aSocietyRoot)
        throw new Error('aSocietyRoot is required');
 ...
 */
export function scaffoldFromManifestFile(projectRoot, projectName, aSocietyRoot, manifestFilePath, options = {}) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const entries = includeOptional ? d.files : d.files.filter((f) => f.required === true);
