 ...
 */
export function createConsentFile(adocsPath, feedbackType, projectName, consentValue, options = {}) {
    if (!adocsPath)
        throw new Error('adocsPath is required');
    if (!projectName)
 ...
 */
export function checkConsent(adocsPath, feedbackType) {
    if (!adocsPath)
        throw new Error('adocsPath is required');
    if (!FEEDBACK_TYPES[feedbackType]) {
        throw new Error(`Unknown feedback type: "${feedbackType}". Valid types: ${Object.keys(FEEDBACK_TYPES).join(', ')}`);
    ...
    const consented = VALID_CONSENT_VALUES.includes(raw)
        ? raw
        : 'unknown';
