...
export class ImprovementOrchestrator {
    static async handleForwardPassClosure(flowRun, signal, inputStream, outputStream) {
        ...
        for (let i = 0; i < plan.length; i++) {
            ...
            await Promise.all(group.map(async (entry, groupIndex) => {
                ...
                if (entry.stepType === 'meta-analysis') {
                    ...
                    const { bundleContent } = ContextInjectionService.buildContextBundle(roleKey, flowRun.projectRoot, [metaAnalysisInstructionPath, ...findingsFilePaths], null);
                    ...
                    const result = await runInteractiveSession(flowRun.projectRoot, roleKey, bundleContent, [{ role: 'user', content: userMessage }], inputStream, outputStream);
                    ...
                    const { bundleContent } = ContextInjectionService.buildContextBundle(roleKey, flowRun.projectRoot, [synthesisInstructionPath, ...allFindingsFiles], null);
                    ...
                    await runInteractiveSession(flowRun.projectRoot, roleKey, bundleContent, [{ role: 'user', content: userMessage }], inputStream, outputStream);
                }
