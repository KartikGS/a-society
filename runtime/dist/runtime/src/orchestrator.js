...
export function parseWorkflow(filePath) {
    ...
    if (!match)
        throw new Error('No valid frontmatter found in workflow');
    return yaml.load(match[1]);
}
export class FlowOrchestrator {
    async startUnifiedOrchestration(workspaceRoot, roleKey, inputStream = process.stdin, outputStream = process.stdout) {
        SessionStore.init();
        ...
        if (!flowRun) {
            ...
            const { bundleContent: bootstrapBundle } = ContextInjectionService.buildContextBundle(roleKey, workspaceRoot, [], null, 'bootstrap');
            while (true) {
                try {
                    ...
                    try {
                        handoffResult = await runInteractiveSession(workspaceRoot, roleKey, bootstrapBundle, bootstrapHistory.length > 0 ? bootstrapHistory : undefined, inputStream, outputStream, true, // autonomous
                    ...
                    if (!artifactPath)
                        throw new Error("Initial interactive session did not supply an artifact_path to locate workflow.md.");
                    ...
                    try {
                        ...
                        if (doc.workflow && doc.workflow.nodes) {
                            ...
                            const node = doc.workflow.nodes.find((n) => n.role === startingRoleName);
                            if (node) {
                                startNodeId = node.id;
                            }
                            else {
    ...
    async advanceFlow(flowRun, nodeId, activeArtifactPath, humanInput, inputStream = process.stdin, outputStream = process.stdout) {
        ...
        const resolvedArtifacts = activeArtifactPath !== undefined
        ...
        const { bundleContent, contextHash } = ContextInjectionService.buildContextBundle(roleKey, flowRun.projectRoot, resolvedArtifacts, null);
        ...
        while (true) {
            try {
                ...
                let handoffResult = null;
                try {
                    handoffResult = await runInteractiveSession(flowRun.projectRoot, roleKey, bundleContent, injectedHistory, inputStream, outputStream, true, // autonomous
                    controller.signal // ← externalSignal
                    );
                }
                finally {
                ...
                if (handoffResult) {
                    if (handoffResult.kind === 'forward-pass-closed') {
                        await ImprovementOrchestrator.handleForwardPassClosure(flowRun, { recordFolderPath: handoffResult.recordFolderPath, artifactPath: handoffResult.artifactPath }, inputStream, outputStream);
                        return;
                    }
    ...
    async applyHandoffAndAdvance(flowRun, nodeId, handoffs) {
        ...
        const wf = parseWorkflow(path.join(flowRun.recordFolderPath, 'workflow.md')).workflow;
        const outgoingEdges = (wf.edges || []).filter((e) => e.from === nodeId);
        ...
        if (outgoingEdges.length === 1) {
            ...
            const successorNode = wf.nodes.find((n) => n.id === edge.to);
            if (!successorNode)
                throw new Error(`Successor node '${edge.to}' not found.`);
        ...
        if (outgoingEdges.length > 1) {
            ...
            const roles = outgoingEdges.map((e) => wf.nodes.find((n) => n.id === e.to)?.role);
            ...
            const claimedHandoffs = new Set();
            for (const edge of outgoingEdges) {
                const targetNode = wf.nodes.find((n) => n.id === edge.to);
    ...
    removeActiveNode(flowRun, nodeId) {
        flowRun.activeNodes = flowRun.activeNodes.filter(id => id !== nodeId);
        delete flowRun.pendingNodeArtifacts[nodeId];
    }
    activateOrDefer(flowRun, wf, candidateNodeId, incomingArtifacts) {
        const incomingEdges = (wf.edges || []).filter((e) => e.to === candidateNodeId);
        ...
        const allComplete = incomingEdges.every((e) => flowRun.completedNodes.includes(e.from));
        if (allComplete) {
            const joinArtifacts = incomingEdges
                .map((e) => flowRun.completedNodeArtifacts[e.from])
                .filter((a) => a !== '');
