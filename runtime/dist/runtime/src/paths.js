 ...
 */
export function resolveVariableFromIndex(variable, projectRoot) {
    ...
    if (internalRes)
        return path.join(projectRoot, internalRes);
    const publicRes = extractPath(publicIndex);
    if (publicRes)
        return path.join(projectRoot, publicRes);
    return null;
}
