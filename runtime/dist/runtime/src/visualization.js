export function renderFlowStatus(flowRun, wf) {
...
function findPendingJoins(flowRun, wf) {
    const incomingEdges = {};
    ...
    return wf.nodes
        ...
        if (preds.length <= 1)
            return false;
        if (flowRun.activeNodes.includes(n.id))
            return false;
        if (flowRun.completedNodes.includes(n.id))
            return false;
