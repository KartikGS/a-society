...
test('renderFlowStatus: single active node, no completed', () => {
  const flowRun: FlowRun = {
    ...
    pendingNodeArtifacts: { 'start': ['p.md'] }, pendingHandoff: [], receivingHandoff: {}, awaitingHandoff: [],
    status: 'running',
    stateVersion: '7'
  };
...
test('renderFlowStatus: multiple active, multiple completed, pending join', () => {
    const flowRun: FlowRun = {
      ...
      pendingNodeArtifacts: { 't1': ['p3.md'] }, pendingHandoff: [], receivingHandoff: {}, awaitingHandoff: [],
      status: 'running',
      stateVersion: '7'
    };
...
test('renderFlowStatus: multiple active nodes renders all', () => {
  const flowRun: FlowRun = {
    ...
    pendingNodeArtifacts: { 't1': ['p2.md'], 't2': ['p3.md'] }, pendingHandoff: [], receivingHandoff: {}, awaitingHandoff: [],
    status: 'running',
    stateVersion: '7'
  };
  const output = renderFlowStatus(flowRun, WF);
