...
When exactly one node should receive control, emit a single handoff object. When parallel branches should start, emit one handoff entry per target node. The handoff syntax itself is defined in `runtime/HANDOFF-CONTRACT.md`; this file defines how those target node ids relate to `workflow.yaml`.
